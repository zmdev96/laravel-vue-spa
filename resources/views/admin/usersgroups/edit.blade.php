  ...
  <div class="container-fluid">
          ...
          <div class="card shadow mb-4">
            <div class="card-header">
              <h6 class=" font-weight-bold text-primary">Users Groups Edit</h6>
              ...
              <div class="row">
                <div class="col-md-10">
                  <div class="form-content">
                    <form class="" action="{{route('admin.usersgroups.update', $usersgroup->id)}}" method="post">
                      {{ csrf_field() }}
                      {{method_field('PUT')}}
                      ...
                      <div class="form-row">
                        <div class="form-group col-md-6 {{ $errors->has('name') ? ' has-error' : '' }}">
                          ...
                          <input type="text" name="name" class="form-control" id="name" value="{{old('name', $usersgroup->name)}}">
                          @if ($errors->has('name'))
                      ...
                      <div class="privileges-list">
                        @if (false !== $privileges) @foreach ($privileges as $privi)
                          <div class="singel-privi">
                            <input type="checkbox" name="privi[]" value="{{$privi->id}}" id="{{$privi->id}}" {{ in_array($privi->id , $group_privi) ? 'checked' : ''}}>
